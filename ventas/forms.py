...
class CalculadoraBilletesForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        print(f"Los valores que entraron: {un_peso}, {tres_pesos}, {cinco_pesos}, {diez_pesos}, {veinte_pesos}, {cincuenta_pesos}, {cien_pesos}, {doscientos_pesos}, {quinientos_pesos}, {mil_pesos}")
        ...
        print(f"------ Se va a iniciar comparacion: tipo_cuenta es: {tipo_cuenta}")
        validar = str(tipo_cuenta) == "Salida"
        print(f"------ la compracioon dio: {validar}")
        if str(tipo_cuenta) == "Salida":
            print("---- es un Debito, se va a comprobar todo")
            print(f"Se va a comprara un_peso: {un_peso}, con el bulto total: {cuenta.un_peso}")
            if un_peso is not None and un_peso > cuenta.un_peso :
                print("hay un error, no hay dinero para pagar eso")
